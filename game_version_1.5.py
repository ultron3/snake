...
#funzione per inserire il punteggio
def save_score(nickname, score):
    sql = "INSERT INTO snake_game.giocatore (nickname, score) VALUES (%s, %s)"
    val = (nickname, score)
    mycursor.execute(sql, val)
    mydb.commit()
#funzione per aggiornare il punteggii
def update_score(nickname, score):
    sql = "UPDATE snake_game.giocatore SET score = %s WHERE nickname = %s"
    val = (score, nickname)
    mycursor.execute(sql, val)
    mydb.commit()
#funzione per selezionare il punteggio secondo il giocatore
def get_score(nickname):
    sql = "SELECT score FROM snake_game.giocatore WHERE nickname = %s"
    val = (nickname,)
    mycursor.execute(sql, val)
...
def gameLoop(nickname):
    ...
    while not game_over:
        ...
        while game_close == True:
            dis.fill(blue)
            message("Hai perso. Premi C per riniziare o Q per uscire", red, [dis_width / 6, dis_height / 3])
...
def game_menu():
    ...
    while menu:
        ...
        message("Premi L per  classifica", black, [dis_width / 3, dis_height / 1])
        pygame.display.update()
